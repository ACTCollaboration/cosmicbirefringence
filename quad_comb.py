...
for q in p.quad.qlist:
    ...
    cb = np.zeros((p.snmax,2,p.quad.bn))
    mcb0, kx0, ik0, vcb0 = np.loadtxt(p0.quad.f[q].mcbs,unpack=True,usecols=(1,3,4,5))
    mcb1, kx1, ik1, vcb1 = np.loadtxt(p1.quad.f[q].mcbs,unpack=True,usecols=(1,3,4,5))
    ...
    for i in range(p.snmax):
        # linear combination
        cb0 = s0**2 * np.loadtxt(p0.quad.f[q].cl[i],unpack=True)[1]
        cb1 = s1**2 * np.loadtxt(p1.quad.f[q].cl[i],unpack=True)[1]
    ...
    if p.snmin==0:
        print('save real')
        np.savetxt(p.quad.f[q].ocbs,np.concatenate((p.quad.bc[None,:],cb[0,:,:],np.std(cb[1:,:,:],axis=0))).T)
