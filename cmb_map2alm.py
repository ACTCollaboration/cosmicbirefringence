...
def cmbalm2cl(p,f,r):
    #//// compute aps ////#
    # output is ell, TT, EE, BB, TE

    cbs = np.zeros((p.snmax,4,p.bn))
    cls = np.zeros((p.snmax,4,p.lmax+1))
    ...
    if p.snmin==0:
        if p.doreal:
            print('save real')
            np.savetxt(f.rcl,np.concatenate((r.eL[None,:],cls[0,:,:])).T)
            np.savetxt(f.rcb,np.concatenate((r.bc[None,:],cbs[0,:,:])).T)
        else:
            print('save mock obs')
